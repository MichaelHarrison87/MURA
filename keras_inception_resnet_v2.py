...
batch_size = 512
num_epochs = 2
num_steps_per_epoch = math.floor(num_images_train/batch_size)  # Use entire dataset per epoch; round up to ensure entire dataset is covered if batch_size does not divide into num_images
num_steps_per_epoch_valid = math.floor(num_images_valid/batch_size)   # As above
...
with tf.Session(config=config) as sess:
    ...
    # Build the model
    model = inception_resnet_v2.build_inception_resnet_v2_notop(image_dimensions = (150, 150, 3)
