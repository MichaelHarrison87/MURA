...
### Models List
models_list = pd.read_csv('./results/models_list.csv')

for index,row in models_list.iterrows():

    model_name = row.Model_Name ## ENSURE CORRECT
    image_path = row.ImagePaths
    image_depth = row.ImageDepth
    ...
    # Images Directory
    dir_images = "./data/processed/" + image_path + "/" ## ENSURE CORRECT. Inception-ResNet-v2 min size is 139x139
    ...
    # Save Studies Summary Data
    dir_studies_summary = './results/studies_summaries/'
    studies_summary_valid.to_csv(dir_studies_summary + model_name + '.csv', index=False)
    print("STUDIES SUMMARY SAVED")
    ...
    cohen_kappa_valid_long = [cohen_kappa_valid]*len(false_positive_valid)
    roc_curve_df = pd.DataFrame({'FalsePositive':false_positive_valid
        , 'TruePositive':true_positive_valid
        , 'Thresholds':thresholds_valid
        , 'AUC': auc_valid_long
        , 'CohensKappa': cohen_kappa_valid_long})
    dir_roc_curve = './results/roc_curve_data/'
